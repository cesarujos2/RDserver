...
function formatterDataTefi(data: any) {
    ...
    data["entry_list"].forEach((element: any, index: any) => {
        let claveRelationShip = Object.keys(data['relationship_list'][index]["link_list"][0]["records"][0]["link_value"])
        ...
        for (let i = 0; i < claveRelationShip.length; i++) {
            dataForm[claveRelationShip[i]] = data['relationship_list'][index]["link_list"][0]["records"][0]["link_value"][claveRelationShip[i]].value
        }
